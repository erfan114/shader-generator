...
import { output } from "@/builder/nodes/output.node.js";
...
describe("Output", () => {
  ...
  it("Should become flatten", () => {
    ...
    expect(targetOutput.data.flatten).toBeUndefined();

    const flattenOutput = targetOutput.flat();

    expect(flattenOutput.data.flatten).toBe(true);
  });
});
